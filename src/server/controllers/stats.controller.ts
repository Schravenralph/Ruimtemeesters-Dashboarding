...
export async function getTimeSeriesAgg(req: Request, res: Response): Promise<void> {
  ...
  const sourceQueries: Record<string, string> = {
    bevolking: `
      WITH ranked AS (
        SELECT year, value, source, confidence_lower, confidence_upper, ROW_NUMBER() OVER (
          PARTITION BY year ORDER BY ${SOURCE_PRIORITY}
        ) as rn
        FROM data_bevolking
        WHERE geo_code = $1 AND age_group = 'totaal' AND gender = 'totaal'
      )
      SELECT year, value as total, source, confidence_lower, confidence_upper FROM ranked WHERE rn = 1 ORDER BY year`,
    huishoudens: `
      WITH ranked AS (
        SELECT year, value, source, confidence_lower, confidence_upper, ROW_NUMBER() OVER (
          PARTITION BY year ORDER BY ${SOURCE_PRIORITY}
  ...
  res.json({
    timeSeries: result.rows.map(r => ({
      ...
      source: r.source,
      ...(r.confidence_lower != null && r.confidence_upper != null
        ? { confidenceLower: Number(r.confidence_lower), confidenceUpper: Number(r.confidence_upper) }
        : {}),
    })),
  });
}
